<style type="text/css"> .tombol-kanan {margin-top: 1px; margin-right: 5px; float: right;}</style>
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li class="active">Jenis Penyakit</li>
      </ol>
    </section>
          ...
          <div class="box box-success">
            ...
            <div class="box-body">
            	<div class="alert alert-success alert-dismissable">
            		...
            		<h4>Pastikan Extensi File Excel yang digunakan Excel 2003-2007 (.xls) untuk format excel anda bisa download <a href="Data_Jenis.xls">di sini</a></h4>
            	</div>
...
//       membaca data (kolom ke-1 sd terakhir)
    	$kd_jenis = $data->val($i, 1);
	 	$jenis_penyakit = $data->val($i, 2);

	 	$cari = mysqli_num_rows(mysqli_query($connect, "SELECT kd_jenis FROM tb_jenispenyakit WHERE kd_jenis = '$kd_jenis'"));

		if (empty($kd_jenis)){
		  echo "<script language='javascript'>window.alert('Impor Gagal, Kode Jenis Tidak Boleh Kosong.'), window.location = '?page=impor-jenis';</script>";
		 }
		 elseif ($cari > 0){
		  echo "<script language='javascript'>window.alert('Impor Gagal, Kode Jenis Ada Yang Sama.'), window.location = '?page=impor-jenis';</script>";
		 }
		 else{
		  // setelah data dibaca, sisipkan ke dalam tabel tsukses
		  $hasil = mysqli_query($connect, "INSERT INTO tb_jenispenyakit(kd_jenis,jenis_penyakit) VALUES('$kd_jenis','$jenis_penyakit')");
...
echo "<script language='javascript'>window.alert('Impor Complete, Data berhasil di tambah.'), window.location = '?page=jenis_penyakit';</script>";
}
