          ...
          <div class="box box-success">
            ...
            <div class="box-body">
            	<div class="alert alert-success alert-dismissable">
            		...
            		<h4>Pastikan Extensi File Excel yang digunakan Excel 2003 (.xls) untuk format excel anda bisa download <a href="Data_Gejala.xls">di sini</a></h4>
            	</div>
...
//       membaca data (kolom ke-1 sd terakhir)
    $kd_gejala = $data->val($i, 1);
	 	$nama_gejala = $data->val($i, 2);

	 	$cari = mysqli_num_rows(mysqli_query($connect, "SELECT kd_gejala FROM tb_gejala WHERE kd_gejala = '$kd_gejala'"));

		if (empty($kd_gejala)){
		  echo "<script language='javascript'>window.alert('Import Gagal, Kode Gejala Tidak Boleh Kosong.'), window.location = '?page=impor-gejala';</script>";
		 }
		 elseif ($cari > 0){
		  echo "<script language='javascript'>window.alert('Import Gagal, Kode Gejala Ada Yang Sama.'), window.location = '?page=impor-gejala';</script>";
		 }
		 else{
		  // setelah data dibaca, sisipkan ke dalam tabel tsukses
		  $hasil = mysqli_query($connect, "INSERT INTO tb_gejala(kd_gejala,nama_gejala) VALUES('$kd_gejala','$nama_gejala')");
...
echo "<script language='javascript'>window.alert('Import Complete, Data berhasil di tambah.'), window.location = '?page=gejala';</script>";
}
